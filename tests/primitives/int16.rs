...
fn int16_array_single_row_reading() {
    ...
    server.exec(&format!("INSERT INTO {table} VALUES ([-5,42])"));
    let schema = Schema::from_type_strings(&[("value", "Array(Int16)")]).unwrap();
...
fn int16_array_multi_row_reading() {
    ...
    for format in FORMATS {
        ...
        assert_eq!(
            decoded,
            vec![
                vec![Value::Array(vec![Value::Int16(-5), Value::Int16(42)])],
...
#[test]
fn int16_array_nullable_single_row_reading() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec(&format!(
        "CREATE TABLE {table} (value Array(Nullable(Int16))) ENGINE=Memory"
    ));
    server.exec(&format!("INSERT INTO {table} VALUES ([NULL,-5])"));
    let schema = Schema::from_type_strings(&[("value", "Array(Nullable(Int16))")]).unwrap();
    ...
    for format in FORMATS {
        ...
        assert_eq!(
            ...
            vec![vec![Value::Array(vec![
                Value::Nullable(None),
                Value::Nullable(Some(Box::new(Value::Int16(-5)))),
            ])]]
        );
    }
}
...
#[test]
fn int16_array_nullable_multi_row_reading() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec(&format!(
...
#[test]
fn int16_array_nullable_single_row_writing() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec(&format!(
...
#[test]
fn int16_array_nullable_multi_row_writing() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec(&format!(
...
#[test]
fn int16_array_low_cardinality_single_row_reading() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec_with_settings(
        &format!("CREATE TABLE {table} (value Array(LowCardinality(Int16))) ENGINE=Memory"),
        "allow_suspicious_low_cardinality_types=1",
    );
    server.exec(&format!("INSERT INTO {table} VALUES ([-5,42])"));
    let schema = Schema::from_type_strings(&[("value", "Array(LowCardinality(Int16))")]).unwrap();
...
#[test]
fn int16_array_low_cardinality_multi_row_reading() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec_with_settings(
...
#[test]
fn int16_array_low_cardinality_single_row_writing() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec_with_settings(
...
#[test]
fn int16_array_low_cardinality_multi_row_writing() {
    let server = ClickhouseServer::connect();
    let table = unique_table("");
    server.exec(&format!("DROP TABLE IF EXISTS {table}"));
    server.exec_with_settings(
